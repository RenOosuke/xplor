...
type file_explorer = {
    getFileIconByType: (fileDirent: Dirent) => string
    ignored: {
        get: () => {
            by_path: (parentPath: string) => boolean;

            all: () => {}

            by_name: (fileName: string) => boolean;

            by_ext: (fileName: string) => boolean;
            
            by_file_path: (parentPath: string) => boolean;
        },
        
        remove: {
            by_name: (fileName: string) => ignoredJSONObj | void
            
            by_path: (filePath: string) => ignoredJSONObj | void

            by_ext: (fileName: string) => ignoredJSONObj | void

            by_file_path: (filePath: string) => ignoredJSONObj | void
        },
        ...
        add: {
            ...
            get: (section: keyof ignoredJSONObj) => {}
        }
    },
    getFileType: (fileDirent: Dirent) => string,
    getDirentFromPath: (filePath: any) => Dirent,
    isDrive: (filePath: any) => boolean,
    getGridLengthPerRow: () => number,
    gridLength: number | undefined,
    interruptFileSelection: boolean,
    menu: (menuType: string, passedMethods: {}, currentDirectory: string, coords: [number, number]) => void,
    crypto: {
        encrypt: (fileData: any, encryptionKey: any) => Buffer;
        decrypt: (encryptedData: any, encryptionKey: any) => Buffer;
        decryptFile: (inputFilePath: any, encryptionKey: any) => string;
        encryptFile: (filePath: any, fileData: any, encryptionKey: any) => any;
    },
    createIfNotExists: (parentDir: string | PathLike) => void;
    getJSON: (filePath: PathLike, encrypted: boolean) => object;
    saveJSON: (filePath: PathLike, encrypted: boolean) => boolean | undefined;
}
